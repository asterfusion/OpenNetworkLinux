...
int set_opt (int fd, int nSpeed, int nBits, char nEvent, int nStop) {
    ...
    tcflush (fd, TCIOFLUSH);   //刷新收到的数据但是不清空
...
//    core = cvmx_get_core_num();
    ...
    {
        int fd;
        int nCount = 0, ulen = 0, nTotal = 1;
        int i,j,m,be_zero,loop;
        int ReadByte = 0;

        unsigned char Buffer[2048];
        struct termios oldtio,newtio;
        char *dev = (char*)"/dev/ttyS1";
        if((fd=open(dev,O_RDWR|O_NOCTTY|O_NONBLOCK))<0) //open serial COM1
        {
	...
	if(argc == 2)
		sprintf(&uart_order[5], "%s", argv[1]);
	else if(argc == 3)
		sprintf(&uart_order[5], "%s_%s", argv[1], argv[2]);
	else if(argc == 4)
		sprintf(&uart_order[5], "%s_%s_%s", argv[1], argv[2], argv[3]);
	unsigned short crc_value = CRC16(uart_order, strlen(uart_order));
	char crc[10] = "\0";
	...
	if(argv[1][strlen(argv[1]) - 1] == '7' || argv[1][strlen(argv[1]) - 1] == '9' || argv[1][strlen(argv[1]) - 1] == 'a' || argv[1][strlen(argv[1]) - 1] == 'c')
		return 0;
    	usleep(700000);

	ReadByte = read(fd, Buffer, 256);
#if 1
        tcflush (fd, TCIOFLUSH);   //刷新发出和收到的数据但是不清空
	int read_crc_check = cmp_read_crc(Buffer, ReadByte);
	if(read_crc_check)
	{
		printf("crc read failed\n");
		return -1;
	}
#endif
        if(ReadByte > 0)
        {			
			for (j = 0; j < ReadByte - 2; j++)
			{
